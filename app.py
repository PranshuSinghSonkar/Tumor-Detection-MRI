import os
import uuid

import numpy as np
from flask import Flask, render_template, request
from PIL import Image
from tensorflow import keras  # type: ignore
from werkzeug.utils import secure_filename

from brainT.src.config import IMG_SIZE
from brainT.src.utils import load_class_names, preprocess_pil_image

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
APP_DIR = os.path.join(BASE_DIR, "brainT")

app = Flask(
    __name__,
    template_folder=os.path.join(APP_DIR, "templates"),
    static_folder=os.path.join(APP_DIR, "static"),
)

MODEL_PATH = os.path.join(APP_DIR, "models", "brain_tumor_classifier.keras")
CLASS_NAMES_PATH = os.path.join(APP_DIR, "models", "class_names.json")
UPLOAD_FOLDER = os.path.join(APP_DIR, "static", "uploads")

ALLOWED_EXTENSIONS = {"png", "jpg", "jpeg"}

app.config["UPLOAD_FOLDER"] = UPLOAD_FOLDER
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

model = None
class_names = None


def allowed_file(filename: str) -> bool:
    return "." in filename and filename.rsplit(".", 1)[1].lower() in ALLOWED_EXTENSIONS


def prettify_label(label: str) -> str:
    return str(label).replace("_", " ").replace("-", " ").title()


def load_assets():
    if not os.path.exists(MODEL_PATH) or not os.path.exists(CLASS_NAMES_PATH):
        raise FileNotFoundError("Model files missing")

    loaded_model = keras.models.load_model(MODEL_PATH)
    loaded_class_names = load_class_names(CLASS_NAMES_PATH)
    return loaded_model, loaded_class_names


def get_model():
    global model, class_names
    if model is None or class_names is None:
        model, class_names = load_assets()
    return model, class_names


@app.route("/")
def home():
    return render_template("index.html")


@app.route("/predict", methods=["POST"])
def predict():
    try:
        if "file" not in request.files:
            return "No file uploaded"

        file = request.files["file"]

        if file.filename == "":
            return "No file selected"

        if not allowed_file(file.filename):
            return "Invalid file type"

        original_name = secure_filename(file.filename)
        ext = original_name.rsplit(".", 1)[1].lower()
        unique_name = f"{uuid.uuid4().hex}.{ext}"

        relative_path = os.path.join("static", "uploads", unique_name)
        absolute_path = os.path.join(APP_DIR, relative_path)

        os.makedirs(os.path.dirname(absolute_path), exist_ok=True)
        file.save(absolute_path)

        image = Image.open(absolute_path).convert("RGB")
        batch = preprocess_pil_image(image, IMG_SIZE)

        loaded_model, loaded_class_names = get_model()

        raw_probabilities = loaded_model.predict(batch, verbose=0)[0]
        best_idx = int(np.argmax(raw_probabilities))

        prediction = prettify_label(loaded_class_names[best_idx])
        confidence = round(float(raw_probabilities[best_idx]) * 100, 2)

        probability_items = sorted(
            [
                {
                    "label": prettify_label(label),
                    "score": float(score),
                    "percent": round(float(score) * 100, 2),
                }
                for label, score in zip(loaded_class_names, raw_probabilities.tolist())
            ],
            key=lambda item: item["score"],
            reverse=True,
        )

        return render_template(
            "result.html",
            prediction=prediction,
            confidence=confidence,
            image_file=relative_path,
            probabilities=probability_items,
            uploaded_name=original_name,
        )
    except Exception as e:
        return f"Error occurred: {str(e)}"


if __name__ == "__main__":
    port = int(os.environ.get("PORT", 8080))
    app.run(host="0.0.0.0", port=port)
